...
![winassoc picker (HTML)](docs/sreenshot-html.png)
![winassoc picker (SVG)](docs/screeshot-svg.png)
...
引数なしで `winassoc.exe` を起動すると、Windows 11 の Mica 効果と Fluent Design 2 に準拠したモダンな設定画面が起動します。

- **「適用」ボタンの一体化**: 「保存」と「適用」を統一し、適用ボタンを押すだけで設定ファイルの自動更新とレジストリへの同期を一撃で処理します。
- **実行ファイル参照 & 自動抽出**: `...` ボタンから実行ファイルを選択すると、そのファイル名から「アプリ名（ID）」を自動生成・入力し、アイコンも即時抽出してプレビューに反映します。
- **リストのゴミ箱削除**: アプリ定義を一覧リスト上のゴミ箱アイコン（🗑️）から1アクションで削除可能です。
- **カラム幅の連動・永続化**: リストのカラム幅はドラッグで調整でき、すべての画面で連動し、タブ切り替えでも維持されます。
- **レイアウトの堅牢化**: 長い実行ファイルパスやルール文字列が入力されても、横スクロールによる右側のレイアウト崩れやボタンの見切れが発生しないように対策されています。
